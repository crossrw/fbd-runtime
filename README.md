...
* Поддержка архитектур Big-Endian и Little-Endian
...
зависят объем программы и объем используемой ею памяти и скорость расчета схемы. Для небольших встраиваемых микроконтроллеров это может быть важным. В общем случае тип данных должен обеспечивать хранение целого числа со знаком. Если вы не уверены в выборе, то рекомендую остановиться на
...
```c
// размер памяти для индекса элемента (1 или 2)
#define INDEX_SIZE 2
```
...
Начиная с версии 10 определение `SPEED_OPT` больше не используется: оптимизация по времени выполнения включена постоянно.


Определение `USE_HMI` необходимо в случае, когда ваш PLC содержит средства организации интерфейса с человеком. Такими средствами могут быть например экран и кнопки. Будьте внимательны: если вы отключите HMI точки управления будут возвращать неопределенные данные! Пример:
...
Функция `FBDsetProc()` используется для записи значений выходных сигналов (контактов) или для сохранения значений в память NVRAM.
...
Не используйте прямую запись в EEPROM при каждом вызове `FBDsetProc()`: библиотека делает такие вызовы при каждом изменении состояния триггеров, таймеров и генератора. Частая запись в EEPROM может значительно сократить срок ее службы. Решением может быть отложенная запись или использование памяти RAM с питанием от батарейки.
...
Инициализация схемы должна быть выполнена один раз в начале работы. Инициализация может быть выполнена повторно для сброса состояния схемы.
...
Если вызов инициализации схемы выполнен без ошибок и вы имеете необходимый объем RAM, то следующим шагом необходимо выполнить вызов инициализации памяти:
...
Параметр __needReset__ должен быть установлен в значение `TRUE` после записи в контроллер новой схемы. При этом значения всех элементов с "памятью" (точек регулирования, триггеров и генераторов) сбрасываются в состояние "по умолчанию".
